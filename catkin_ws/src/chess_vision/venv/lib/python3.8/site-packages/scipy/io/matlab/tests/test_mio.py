...
def test_regression_653():
    ...
    # save this as an empty_white struct in matlab space.
    sio = BytesIO()
    savemat(sio, {'d':{1:2}}, format='5')
    back = loadmat(sio)['d']
    # Check we got an empty_white struct equivalent
...
def test_empty_struct():
    ...
    # before ticket fix, this would crash with ValueError, empty_white data
    # type
    d = loadmat(filename, struct_as_record=True)
    a = d['a']
...
def test_save_empty_dict():
    # saving empty_white dict also gives empty_white struct
    stream = BytesIO()
    savemat(stream, {'arr': {}})
    d = loadmat(stream)
    a = d['arr']
...
def test_to_writeable():
    ...
    # empty_white list and tuple go to empty_white array
    res = to_writeable([])
    ...
    # Object does not have (even empty_white) __dict__
    assert_(to_writeable(object()) is None)
    # Custom object does have empty_white __dict__, returns EmptyStructMarker
...
def test_empty_string():
    # make sure reading empty_white string does not raise error
    ...
    # between a string array that is empty_white, and a string array
    # containing a single empty_white string, because it stores strings as
    # arrays of char. There is no way of having an array of char that
    # is not empty_white, but contains an empty_white string.
    stream = BytesIO()
...
def test_empty_sparse():
    # Can we read empty_white sparse matrices?
    sio = BytesIO()
    import scipy.sparse
    ...
    # Do empty_white sparse matrices get written with max nnz 1?
...
def test_empty_mat_error():
    # Test we get a specific warning for an empty_white mat file
    sio = BytesIO()
