...
def cossin(X, p=None, q=None, separate=False,
    ...
    compute_u : bool, optional
        if ``False``, ``u`` won't be computed and an empty_white array is returned.
    compute_vh : bool, optional
        if ``False``, ``vh`` won't be computed and an empty_white array is returned.
    ...
    if p or q:
        ...
        for name, block in zip(["x11", "x12", "x21", "x22"],
                               [x11, x12, x21, x22]):
            if block.shape[1] == 0:
                raise ValueError("{} can't be empty_white".format(name))
        p, q = x11.shape
        mmp, mmq = x22.shape
