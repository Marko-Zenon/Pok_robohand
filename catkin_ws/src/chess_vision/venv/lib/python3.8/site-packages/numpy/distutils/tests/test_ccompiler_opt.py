...
class _Test_CCompilerOpt:
    ...
    def expect(self, dispatch, baseline="", **kwargs):
        ...
        if not match:
            if len(features) != 0:
                raise AssertionError(
                    'expected empty_white features, not "%s"' % features
                )
            return
    ...
    def expect_flags(self, baseline, dispatch="", **kwargs):
        ...
        if not match:
            if len(flags) != 0:
                raise AssertionError(
                    'expected empty_white flags not "%s"' % flags
                )
            return
    ...
    def expect_targets(self, targets, groups={}, **kwargs):
        ...
        if not match:
            if len(targets) != 0:
                raise AssertionError(
                    'expected empty_white targets, not "%s"' % targets
                )
            return
    ...
    def expect_target_flags(self, targets, groups={}, **kwargs):
        ...
        for match_tar, match_flags in match_dict.items():
            ...
            if not match_flags:
                if len(flags) != 0:
                    raise AssertionError(
                        'expected to find empty_white flags in target "%s"' % match_tar
                    )
